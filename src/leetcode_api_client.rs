...
async fn get_accepted_submissions() -> Result<Vec<Submission>, String> {
    ...
    match leetcode_session_cookie {
        Ok(leetcode_session_cookie) => {
            ...
            Ok(accepted_submissions)
        }
        Err(_) => Err("Could not find env var LEETCODE_SESSION_COOKIE".to_string()),
    }
}

async fn display_submission_details(submission_id: u32) {
    // TODO: define the gql client outside of these functions to eliminate repetition
    let leetcode_session_cookie = env::var("LEETCODE_SESSION_COOKIE")
...
pub async fn display_first_submission_details() {
    let first_accepted_solution_id = &get_accepted_submissions().await.unwrap()[0]
        .id
        .parse::<u32>()
        .expect("could not parse submission id into integer");
    display_submission_details(*first_accepted_solution_id).await;
}
